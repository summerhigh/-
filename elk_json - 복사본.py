...
def flatten_json(data):
    flattened = []
    for facility in data.values():
        facility_name = facility["시설명"]
        start_date = facility["진단 시작일자"]
        end_date = facility["진단 종료일자"]
        for system in facility["시스템 목록"]:
            system_info = {
                "시설명": facility_name,
                "진단 시작일자": start_date,
                "진단 종료일자": end_date,
                "시스템 이름": system["시스템 이름"],
                "IP 주소": system["IP 주소"],
                "운영 체제": system["운영 체제"],
                "운영 체제 버전": system["운영 체제 버전"],
                "지역": system["지역"]
            }
            for item_id, item in system["진단 항목"].items():
                flattened_item = {**system_info, **{
                    "진단 항목 ID": item_id,
                    "카테고리": item["카테고리"],
                    "항목 설명": item["항목 설명"],
                    "중요도": item["중요도"],
    ...
    json_string = json.dumps(flattened, separators=(',', ':'), ensure_ascii=False)
    return json_string
...
if __name__ == "__main__":
    ...
    # 평탄화된 결과 저장 (진단결과_flattened_1.json 형식)
    flattened_data = flatten_json(merged_results)
