...
def check_ip_port_restrictions():
    try:
        ...
        iptables_check = subprocess.run(['iptables', '-L'], capture_output=True, text=True, stderr=subprocess.DEVNULL)
        iptables_configured = iptables_check.returncode == 0 and 'ACCEPT' in iptables_check.stdout

        # IPFilter, 다른 방화벽 설정 확인 (솔라리스, AIX 등)
        ipfilter_check = subprocess.run(['ipfstat', '-io'], capture_output=True, text=True, stderr=subprocess.DEVNULL)
