...
# /etc/shadow 파일 소유자 및 권한 조치 함수
def remediate_shadow_file_permissions():
    try:
        # 첫 번째 시도: /etc/shadow 파일 소유자를 root로 변경 및 권한을 400으로 설정
        subprocess.run(['chown', 'root', '/etc/shadow'], check=False)
        subprocess.run(['chmod', '400', '/etc/shadow'], check=False)

        # 설정이 정상적으로 완료되었는지 재확인
        shadow_stat = os.stat('/etc/shadow')

        if shadow_stat.st_uid == 0 and oct(shadow_stat.st_mode)[-3:] == '400':
            return "완료"  # 소유자가 root이고 권한이 400으로 설정된 경우 완료
        else:
...
# /etc/shadow 파일 소유자 및 권한 확인 함수 (재진단)
def check_shadow_file_permissions():
    try:
        # 첫 번째 시도: /etc/shadow 파일의 소유자 및 권한 확인
        shadow_stat = os.stat('/etc/shadow')
        ...
        if file_permissions != '400' and file_permissions != '000':
            return {"status": "취약", "message": "파일 권한이 400 이하가 아닙니다."}

        return {"status": "양호", "message": "/etc/shadow 파일의 소유자가 root이고, 권한이 400 이하로 설정되어 있습니다."}
        ...
        try:
            ...
            if 'root' not in result.stdout or 'r--------' not in result.stdout:
                return {"status": "취약", "message": "AIX 또는 다른 시스템에서 소유자가 root가 아니거나 권한이 400 이하가 아닙니다."}
            return {"status": "양호", "message": "/etc/shadow 파일의 소유자가 root이고, 권한이 400 이하로 설정되어 있습니다."}
        except Exception as e:
...
def main():
    ...
    # 먼저 조치를 수행
    remediation_result = remediate_shadow_file_permissions()

    # 조치 수행 시간 기록
    remediation_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    # 재진단 수행
    diagnosis_result = check_shadow_file_permissions()
